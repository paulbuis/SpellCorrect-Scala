object Testing {
    val tests1 : Map[String, String] = Map(
        "access"-> "acess",
        "accessing"-> "accesing",
        ...
        "splendid"-> "spledid splended splened splended", "standardizing"-> "stanerdizing",
    ...
    val tests2 = Map("forbidden"-> "forbiden",
        "decisions"-> "deciscions descisions",
        "supposedly"-> "supposidly", "embellishing"-> "embelishing",
        "technique"-> "tecnique", "permanently"-> "perminantly", "confirmation"-> "confermation",
        "appointment"-> "appoitment", "progression"-> "progresion",
        "accompanying"-> "acompaning",
        "applicable"-> "aplicable", "regained"-> "regined",
        "guidelines"-> "guidlines",
        "surrounding"-> "serounding", "titles"-> "tittles", "unavailable"-> "unavailble",
        "advantageous"-> "advantageos", "brief"-> "brif", "appeal"-> "apeal",
        "consisting"-> "consisiting", "clerk"-> "cleark clerck", "component"-> "componant",
        "favourable"-> "faverable", "separation"-> "seperation", "search"-> "serch",
        "receive"-> "recieve", "employees"-> "emploies", "prior"-> "piror",
        "resulting"-> "reulting", "suggestion"-> "sugestion", "opinion"-> "oppinion",
        "cancellation"-> "cancelation", "criticism"-> "citisum", "useful"-> "usful",
        "humour"-> "humor", "anomalies"-> "anomolies", "would"-> "whould", "doubt"-> "doupt",
        "examination"-> "eximination", "therefore"-> "therefoe", "recommend"-> "recomend",
        "separated"-> "seperated", "successful"-> "sucssuful succesful",
        ...
        "organized"-> "oranised", "accept"-> "acept",
        "dependence"-> "dependance", "unequalled"-> "unequaled",
        "numbers"-> "numbuers", "sense"-> "sence",
        "conversely"-> "conversly",
        "provide"-> "provid", "arrangement"-> "arrangment",
        "responsibilities"-> "responsiblities", "fourth"-> "forth",
        "ordinary"-> "ordenary", "description"-> "desription descvription desacription",
        "inconceivable"-> "inconcievable", "data"-> "dsata", "register"-> "rgister",
        "supervision"-> "supervison", "encompassing"-> "encompasing", "negligible"-> "negligable",
        "allow"-> "alow", "operations"-> "operatins", "executed"-> "executted", "interpretation"-> "interpritation",
        "hierarchy"-> "heiarky",
        "indeed"-> "indead", "years"-> "yesars", "through"-> "throut",
        "committee"-> "committe", "inquiries"-> "equiries", "before"-> "befor", "continued"->
"contuned",
    "permanent"-> "perminant", "choose"-> "chose", "virtually"->
...
"curruntly",
        "forecast"-> "forcast", "weapons"-> "wepons", "routine"-> "rouint",
...
"anual",
        "according"-> "acording", "receives"-> "recives recieves",
...
"easyly", "organization"-> "oranisation oragnisation", "the"-> "thw hte thi",
        ...
        "discipline"-> "disiplin",
        "announcement"-> "anouncement", "progresses"-> "progressess",
        "except"-> "excxept", "recommending"-> "recomending",
        "mathematically"-> "mathematicaly", "source"-> "sorce", "combine"-> "comibine", "input"-> "inut",
        "careers"-> "currers carrers", "resolved"-> "resoved", "demands"-> "diemands",
        "unequivocally"-> "unequivocaly", "suffering"-> "suufering",
        "immediately"-> "imidatly imediatly", "accepted"-> "acepted", "projects"-> "projeccts",
        "necessary"-> "necasery nessasary nessisary neccassary",
        "journalism"-> "journaism", "unnecessary"-> "unessessay",
        "night"-> "nite", "output"-> "oputput", "security"-> "seurity",
        "essential"-> "esential", "beneficial"-> "benificial benficial",
        "explaining"-> "explaning", "supplementary"-> "suplementary",
        "questionnaire"-> "questionare", "employment"-> "empolyment",
        ...
        "accounts"-> "acounts",
        "primarily"-> "pimarily", "operator"-> "opertor",
        ...
        "again"-> "agiin",
        "assessing"-> "accesing",
        "ideally"-> "idealy", "scrutinized"-> "scrutiniesed",
        "simular"-> "similar",
        "personnel"-> "personel",
        "whereas"-> "wheras",
        "when"-> "whn",
...
import spelling.LanguageModel
import spelling.CandidateGenerator
import spelling.CandidateEvaluator1

def spelltest(tests : Map[String, String]) :Unit = {
    val verbose = true
    var n = 0
    var bad = 0
    var unknown = 0
    
    val splitTests = tests.mapValues(wrongs=>wrongs.split(' '));
    for ( (right, wrongs) <- splitTests;
           wrong <- wrongs) {
            n += 1 
            val candidates : CandidateGenerator.CandidateList =
                CandidateGenerator.candidates(wrong)
            val evaluatedCandidateList : List[CandidateEvaluator1.EvaluatedCandidate] =
                CandidateEvaluator1.evaluateCandidates(candidates)            
            
            if (evaluatedCandidateList.length == 0) {
                println("no result for: " + wrong + ", expected: " + right +
                         " (" + LanguageModel.oneGramFrequency(right) +")" )
                println("")
            }
            else {
                val w = evaluatedCandidateList.head._1
                if (w != right) {
                    bad += 1
                    if (!LanguageModel.isWord(right)) {
                        println("unknown: " + right)
                        unknown += 1
                    }
                
                    println("correct(" + wrong + ") => " + w +
                         " (" + LanguageModel.oneGramFrequency(w) + "), expected: " + right +
                         " (" + LanguageModel.oneGramFrequency(right) +")"  )
                    evaluatedCandidateList.map(x=>{println(x._1); println(x._2); println(x._3); println("")} )
                    println("")
                }
        ...
        val percentCorrect = 100.0 - 100.0*bad/n
        println(percentCorrect + "% Correct")  
    }
